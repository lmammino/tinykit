...
pub fn write_sam_env(config: SamEnvConfig) -> Result<()> {
    let template = std::fs::read(&config.template_path)
        .map_err(|err| anyhow::anyhow!("Failed to read template file {:?}: {}", &config.template_path, err))?;
    ...
    if resources.len() != 1 {
        return Err(anyhow::anyhow!("Expect to find exactly one resource with CodeUri ends with {}. Found: {}", config.package_name, resources.len()));
    }
    ...
    let env_properties = resources[0]
        ...
        .map(|v| v.keys().map(|k| k.as_str().expect("Environment Variable key should be a string")).collect::<Vec<_>>());

    eprintln!("{:#?}", env_properties);

    let props = env_properties
        .map(|v| {
            let v: Vec<_> = v.into_iter().map(|k| {
                format!(r##"
#[envconfig(from = "{}")]
{}: String,"##, k, k.to_lowercase())
            }).collect();
            v.join("\n")
        })
        .unwrap_or("".to_string());
